...
const fs = require('fs'); // For checking files
...
// --- ROUTES ---
// FIXED: Serve the HTML file directly
app.get('/', (req, res) => {
    const indexPath = path.join(__dirname, 'public', 'index.html');
    if (fs.existsSync(indexPath)) {
        res.sendFile(indexPath);
    } else {
        res.status(404).send('Index file not found. Please ensure public/index.html exists.');
    }
});
    ...
    data.rooms.forEach((room, roomIdx) => {
        ...
        const table = {
            ...
            rows: room.windows.map((win, i) => [
                ...
                win.width,
                win.height,
                'Standard Configuration - Individual Pane Adjustments Applied'
            ])
        };
        doc.table(table, {
            x: 50,
...
const generateProductionSheets = (doc, data, quote) => {
    ...
    data.rooms.forEach(room => {
        room.windows.forEach((win, wIdx) => {
            ...
            // FIXED: Compute vPositions and rowHeights
            let vPositions = [0];
            if (win.vSplits && win.vSplits.length > 0) {
                vPositions = vPositions.concat(win.vSplits);
            }
            vPositions.push(1);
            vPositions.sort((a, b) => a - b);
            const cutHeight = win.height - timberDeduction;
            let rowHeights = [];
            for (let jj = 0; jj < vPositions.length - 1; jj++) {
                rowHeights.push(cutHeight * (vPositions[jj + 1] - vPositions[jj]));
            }
            const cutWidth = win.width - timberDeduction;
            // FIXED: Compute hPositions global based on first row
            let hPositions = [0];
            if (win.hSplits && win.hSplits.length > 0) {
                hPositions = hPositions.concat(win.hSplits);
            }
            hPositions.push(1);
            hPositions.sort((a, b) => a - b);
            // Window header
            doc.roundedRect(50, doc.y, doc.page.width - 100, 30, 5).fill('#023047');
            doc.fillColor('#FFFFFF').fontSize(11)
                .text(`Room: ${room.name} | Window ${wIdx + 1} | Style: ${win.style}`, 65, doc.y + 8);
            doc.moveDown(1.5); // IMPROVED: Added space
            const rows = win.style.split('/');
            const numRows = rows.length;
            // Cutting List - Enhanced
            const cuttingRows = [
                ['Head / Sill', `${cutWidth.toFixed(1)} mm`, 2],
                ['Jambs', `${cutHeight.toFixed(1)} mm`, 2]
            ];
            if (numRows > 1) {
                cuttingRows.push(['Transoms', `${cutWidth.toFixed(1)} mm`, numRows - 1]);
            }
            // Mullions per row
            rows.forEach((row, rIdx) => {
                const panesInRow = row.split('-');
                const numPanesInRow = panesInRow.length;
                const rowH = rowHeights[rIdx] || (cutHeight / numRows);
                if (numPanesInRow > 1) {
                    cuttingRows.push([
                        `Mullions (Row ${rIdx + 1})`,
                        `${rowH.toFixed(1)} mm`,
            ...
            doc.moveDown(1.5); // IMPROVED: Added more space
            // FIXED: Glass Schedule with per-row computations
            const glassRows = [];
            rows.forEach((row, rIdx) => {
                const panes = row.split('-');
                const numPanes = panes.length;
                const rowH = rowHeights[rIdx] || (cutHeight / numRows);
                // Compute row-specific h positions
                let rowHPos = [0];
                if (win.hSplits && win.hSplits.length + 1 === numPanes) {
                    rowHPos = hPositions;
                } else {
                    // Equal split
                    for (let pp = 1; pp <= numPanes; pp++) {
                        rowHPos.push(pp / numPanes);
                    }
                }
                let rowPaneWidths = [];
                for (let pp = 0; pp < rowHPos.length - 1; pp++) {
                    rowPaneWidths.push(cutWidth * (rowHPos[pp + 1] - rowHPos[pp]));
                }
                // Add panes
                panes.forEach((paneType, pIdx) => {
                    if (pIdx < rowPaneWidths.length) {
                        const paneW = rowPaneWidths[pIdx];
                        glassRows.push([
                            `${rIdx + 1}-${pIdx + 1}`,
                            paneTypeMap[paneType] || 'Unknown',
                            `${paneW.toFixed(1)} mm`,
                            `${rowH.toFixed(1)} mm`,
...
// --- FIXED: Internal Calculation Logic with split-based dimensions ---
function calculateInternal(body) {
    ...
    rooms.forEach(room => {
        room.windows.forEach(win => {
            ...
            totalWindows++;
            const timberDeduction = existingFrame === 'timber' ? 60 : 0;
            let cutHeight = win.height - timberDeduction;
            let cutWidth = win.width - timberDeduction;
            if (cutHeight <= 0 || cutWidth <= 0) return;
            const rowsStr = win.style.split('/');
            const numRows = rowsStr.length;
            let materialCost = 0;
            let verticalMullionLength = 0;
            // Frame perimeter
            materialCost += (cutWidth * pricingData.frameHeadCostPerMm) +
                (cutWidth * pricingData.frameSillCostPerMm) +
                (2 * cutHeight * pricingData.frameJambCostPerMm);
            // Transoms
            if (numRows > 1) materialCost += (numRows - 1) * cutWidth * pricingData.mullionCostPerMm;
            // FIXED: Compute vPositions and rowHeights
            let vPositions = [0];
            if (win.vSplits && win.vSplits.length > 0) {
                vPositions = vPositions.concat(win.vSplits);
            ...
            // FIXED: Global hPositions based on first row
            let hPositions = [0];
            if (win.hSplits && win.hSplits.length > 0) {
                hPositions = hPositions.concat(win.hSplits);
            }
            hPositions.push(1);
            hPositions.sort((a, b) => a - b);
            // Per row/pane calculations with individual dims
            rowsStr.forEach((row, rIdx) => {
                if (rIdx >= rowHeights.length) return;
                const rowH = rowHeights[rIdx];
                const panes = row.split('-');
                const numPanesInRow = panes.length;
                // Vertical mullions for this row
                if (numPanesInRow > 1) {
                    verticalMullionLength += (numPanesInRow - 1) * rowH;
                }
                // Compute row-specific pane widths
                let rowPaneWidths = [];
                if (numPanesInRow === 1) {
                    rowPaneWidths = [cutWidth];
                } else {
                    const expectedNumPanes = hPositions.length;
                    if (numPanesInRow === expectedNumPanes) {
                        for (let pp = 0; pp < numPanesInRow; pp++) {
                            rowPaneWidths.push(cutWidth * (hPositions[pp + 1] - hPositions[pp]));
                        }
                    } else {
                        // Equal split
                        for (let pp = 0; pp < numPanesInRow; pp++) {
                            rowPaneWidths.push(cutWidth / numPanesInRow);
                        }
                    }
                }
                // Glass and fixed costs per pane
                panes.forEach((pane, pIdx) => {
                    if (pIdx >= rowPaneWidths.length) return;
                    const paneW = rowPaneWidths[pIdx];
                    const glassArea = rowH * paneW;
                    materialCost += glassArea * pricingData.glassCostPerSqMm * glazingMultiplier;
            ...
            const totalExtrusionLength = (2 * cutWidth) + (2 * cutHeight) +
                ((numRows - 1) * cutWidth) + verticalMullionLength;
...
app.listen(PORT, () => {
    ...
    console.log('Ensure you have public/index.html, caplock-logo.png (optional), and fonts/ directory (optional).');
});